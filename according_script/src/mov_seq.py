...
class GoToPose():
    def __init__(self):
        ...
        self.counting_client = CountingClient()
        self.counting_client.move_forward(duration='Like')
...
class CountingClient:
    ...
    def check_server_availability(self):
        return self.client.wait_for_server(timeout=rospy.Duration(1.0))

    def move_forward(self, duration):
        if not self.server_available:
            if self.check_server_availability():
                rospy.loginfo("CountingClient server is now available.")
                self.server_available = True
            else:
                rospy.loginfo("CountingClient server is still not available.")
                return
